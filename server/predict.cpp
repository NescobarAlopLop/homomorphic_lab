...
void multiply_vectors_serialization_test_files()
{
    // server code:
    {
        ofstream ofs_parameters("parameters", ios::binary);
...
//        auto bit_sizes = {40, 40, 40, 40, 40}; // 84 mb
    ...
    // client code
    {
        ifstream parameters_file_stream("parameters", ios::binary);
        ...
        ofstream ofs_secret_key("secret_key", ios::binary);
        secret_key.save(ofs_secret_key);

        auto relin_keys = key_generator.relin_keys_local();         // TODO: switch to non local
        auto galois_keys = key_generator.galois_keys_local();

        ofstream ifs_relin("relin", ios::binary);
        ofstream ifs_galois("galois", ios::binary);
        ofstream ifs_public_key("public_key", ios::binary);
        ...
        ofstream ofs_encrypted_query("encrypted_query", ios::binary);
    ...
    // server code
    {
        ...
        ifstream ifs_relin("relin", ios::binary);
        ifstream ifs_galois("galois", ios::binary);
        ifstream ifs_public_key("public_key", ios::binary);

        PublicKey server_public_key;
        server_public_key.load(server_context, ifs_public_key);
        Encryptor encryptor(server_context, server_public_key);

        ifstream ifs_encrypted_query("encrypted_query", ios::binary);
        Ciphertext encrypted_query;
        ...
        ofstream ofs_result("result", ios::binary);
    ...
    {
        EncryptionParameters parms;
        ifstream ifs_parameters("parameters", ios::binary);
        ...
        */
        SecretKey sk;
        ifstream ifs_secret_key("secret_key", ios::binary);
        sk.load(context, ifs_secret_key);
        Decryptor decryptor(context, sk);
        CKKSEncoder encoder(context);

        Ciphertext encrypted_result;
        ifstream ifs_result("result", ios::binary);
...
void predict()
{
    ...
    */
    ...
    with zeros to full size (poly_modulus_degree / 2) when encoding.
    */
    vector<double> input_features{0.0, 1.1, 2.2, 3.3 };
//    vector<double> input_features{0.0, 1.1, 2.2, 3.3 };
    cout << "Input vector: " << endl;
    ...
    */
    ...
    print_line(__LINE__);
    cout << "Encode input vector." << endl;
    encoder.encode(input_features, scale, plain);
